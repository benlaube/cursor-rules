...
```mermaid
flowchart TD
    Start([Start project audit]) --> Structure[Validate structure\n(root, docs, .cursor, env files)]
    Structure --> Modules[Inspect modules\nlayout & required files]
    Modules --> DB[Database & architecture review\n(RLS/comments if deep=true)]
    DB --> Hygiene[Code hygiene checks\nfile size, secrets, docs]
    Hygiene --> Docs[Documentation validation\nREADME/roadmap/architecture]
    Docs --> Report[Summarize findings\nper checklist]
    Report -->|fix=true| Remediate[Apply safe auto-fixes]
    Report -->|fix=false| Output([Output report for follow-up])
    Remediate --> Output
```
...
_If `deep=true` and Supabase detected, apply `.cursor/rules/supabase-rls-policy-review.mdc` rule_
