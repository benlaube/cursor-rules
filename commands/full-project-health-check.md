...
- `output`: `console` (default) or `file`. If `file`, saves report to `/docs/audit/project_health_audit_MM-DD-YYYY.md` (see audit-report rule for naming format).
...
```mermaid
flowchart TD
    Start([Run full health check]) --> ProjectAudit[Run project-audit\n(deep=true)]
    ProjectAudit --> Security[audit-security]
    Security --> RLS[Supabase RLS review?\nIf Supabase detected run policy review]
    RLS --> Aggregate[Aggregate results\n(executive summary + details)]
    Aggregate --> Output[Generate combined report\n(console or file)]
    Output --> Actions[Produce action items\nprioritized by severity]
```
...
**Note:** This command follows the audit report standards. Ensure `/docs/audit/` directory exists before generating report.

If `output=file`, saves to `/docs/audit/project_health_audit_MM-DD-YYYY.md`:
...
**Generated:** MM-DD-YYYY HH:MM:SS EST
**Auditor:** AI Agent
**Scope:** Comprehensive project health check including project audit, security audit, and RLS policy review
**Overall Status:** ⚠️ Needs Attention
